...
class White(Static):
    ...
    where:
    未(.,.) is the Kronecker delta,
    蟽虏  is the variance parameter.
    """
...
class Constant(Static):
    ...
    are constant, i.e. f(x) = c, with c ~ N(0, 蟽^2). The kernel equation is
...
class Stationary(Kernel):
    ...
    def scaled_euclid_dist(self, X, X2):  # pragma: no cover
        """
        Returns |(X - X2岬€)/lengthscales| (L2-norm).
        """
    ...
    def K(self, X, X2=None, presliced=False):
        ...
        where r虏 = ((x - x')/lengthscales)虏.

        Internally, this calls self.K_r2(r虏), which in turn computes the
...
class SquaredExponential(Stationary):
    ...
    where:
    r   is the Euclidean distance between the input points, scaled by the lengthscale parameter 鈩?
    蟽虏  is the variance parameter
...
class RationalQuadratic(Stationary):
    ...
    r  is the Euclidean distance between the input points, scaled by the lengthscale parameter 鈩?
    蟽虏 is the variance parameter,
    伪  determines relative weighting of small-scale and large-scale fluctuations.

    For 伪 鈫?鈭? the RQ kernel becomes equivalent to the squared exponential.
    """
...
class Periodic(Kernel):
    ...
    The resulting kernel can be expressed as:
        k(r) =  蟽虏 exp{ -0.5 sin虏(蟺 r / 纬) / 鈩撀瞹

    where:
    r  is the Euclidean distance between the input points
    鈩?is the lengthscale parameter,
    蟽虏 is the variance parameter,
    纬 is the period parameter.
